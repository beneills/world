...
char* world_render(world* w) {
  ...
  for ( unsigned int y = 0,
    ...
    for ( unsigned int x = 0;
      ...
      } else if ( ( output[i] = render_map_at_node(w->m, x, y) ) ) {
      } else {
	output[i] = GRAPHICS_MAP_EMPTY_CHAR;
      }

      // possibly add newline
      if ( x % w->m->width == w->m->width - 1 ) {
	output[i++] = '\n';
      }
    }
  }
...
char render_fauna_at_node(fauna* head, unsigned int x, unsigned int y) {
  // TODO
  node n = {
    .x = x,
    .y = y
  };
  return fauna_at(head, n) ?
